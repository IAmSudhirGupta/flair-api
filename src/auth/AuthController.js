...
router.post("/register", function (req, res) {
  User.findOne({ email: req.body.email }, function (err, user) {
    if (user)
      return res
        .status(400)
        .send({ status: "error", message: "This email is already exist." });
...
router.post("/register/device", async function (req, res) {
  try {
    ...
    if (!req.body.deviceId)
      res
        .status(400)
        .send({ status: "error", message: "deviceId not present." });
    const query = { deviceId: req.body.deviceId };
    const update = { $set: req.body };
    const options = { upsert: true };
    db.collection("device").updateOne(query, update, options);
    res
      .status(200)
...
router.post("/login", function (req, res) {
  User.findOne({ email: req.body.email }, function (err, user) {
    if (err)
      return res
        .status(500)
        .send({ status: "error", message: "Error on the server." });
    if (!user)
      return res
        .status(404)
        .send({ status: "error", message: "No user found." });
    if (!req.body.password)
      return res
        .status(400)
        .send({ status: "error", message: "password empty." });
...
router.post("/reset-password", verifyToken, function (req, res) {
  User.findById(req.body.id, function (err, user) {
    ...
    if (!passwordIsValid)
      return res
        .status(400)
        .send({ status: "error", message: "Old password is wrong." });
...
router.post("/verifyotp/:id", function (req, res) {
  User.findById(req.params.id, function (err, user) {
    ...
    if (!req.body.otp) {
      return res
        .status(400)
        .send({ status: "error", message: "OTP not found." });
    }
...
router.post("/setpassword/:id", function (req, res) {
  User.findById(req.params.id, function (err, user) {
    ...
    if (!req.body.newPassword)
      return res
        .status(400)
        .send({ status: "error", message: "Password is not provided." });
